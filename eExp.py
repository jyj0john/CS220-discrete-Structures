...
# precondition: left>0 AND right>0
  ...
  while (k > 0) : # the loop test
    assert (k > 0) and invariant(n, k, e, left, right) # fill in the proper loop test and loop invariant
    print("   n:",n,"k:",k,"e:",e)
    # body
    if(k%2 != 0):
      e = e*n
  ...
  assert (k<=0) and invariant(n, k, e, left, right) # fill in proper not(loop test) and loop invariant
  return e
